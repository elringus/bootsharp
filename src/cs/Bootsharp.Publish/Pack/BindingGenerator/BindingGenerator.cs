...
internal sealed class BindingGenerator (Preferences prefs, bool debug)
{
    ...
    private void EmitImports ()
    {
        builder.Append(
            ...
            import { registerImported, getImportedId, getImported, registerExported, getExported } from "./instances";
    ...
    private void EmitEventExport (EventMeta evt)
    {
        ...
        if (instanced)
        {
            var invName = $"{BuildJSInstanceClassName(instance!)}.broadcast{evt.Name}";
            args = PrependIdArg(args);
            invArgs = PrependIdArg(invArgs);
    ...
    private void EmitPropertyExport (PropertyMeta prop)
    {
        ...
        if (prop.CanGet)
        {
            ...
            var body = Deserialize(prop.Value, instanced ? $"{invName}(_id)" : $"{invName}()");
        ...
        if (prop.CanSet)
        {
            ...
            var body = instanced ? $"{invName}(_id, {value})" : $"{invName}({value})";
    ...
    private void EmitPropertyImport (PropertyMeta prop)
    {
        ...
        if (prop.CanGet)
        {
            ...
            var body = Serialize(prop.Value, instanced ? $"getImported(_id).{prop.JSName}" : $"this.{prop.JSName}");
        ...
        if (prop.CanSet)
        {
            ...
            var body = instanced ? $"getImported(_id).{prop.JSName} = {value}" : $"this.{prop.JSName} = {value}";
    ...
    private void EmitMethodExport (MethodMeta method)
    {
        ...
        if (instanced) args = PrependIdArg(args);
    ...
    private void EmitMethodImport (MethodMeta method)
    {
        ...
        var invName = instanced ? $"getImported(_id).{name}" : $"this.{name}Handler";
        var body = Serialize(method.Value, $"{(wait ? "await " : "")}{invName}({invArgs})");
        var serdeHandler = $"{(wait ? "async " : "")}({args}) => {body}";
        if (instanced) builder.Append($"{Br}{name}Serialized: {serdeHandler}");
        else
        {
    ...
    private string Deserialize (ValueMeta value, string exp)
    {
        if (value.InstanceType is { } it)
        {
            ...
            return instance.Interop == InteropKind.Import
                ? $"getImported({exp})"
                : $"new {BuildJSInstanceClassName(instance)}({exp})";
        }
