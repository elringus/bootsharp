...
internal sealed class InspectionReporter (TaskLoggingHelper logger)
{
    public void Report (SolutionInspection inspection)
    {
        ...
        logger.LogMessage(MessageImportance.Normal, JoinLines("Discovered interop members:",
            JoinLines(GetDiscoveredMembers(inspection))));
    ...
    {
        return inspection.StaticMethods.Select(m => m.Assembly)
