...
describe("while bootsharp is booted", () => {
    ...
    it("can transfer array of strings", async () => {
        ...
        assert.deepStrictEqual(Test.testArrayArgFunction(["a", "b"]), ["a", "b"]);
    });
    ...
    it("can transfer and decode raw bytes", () => {
        ...
        assert.strictEqual(Test.bytesToString(bytes), "Everything's shiny, Captain. Not to fret.");
    });
    it("can catch js exception", () => {
        Test.throwJS = function () { throw new Error("foo"); };
        assert.strictEqual(Test.catchException().split("\n")[0], "Error: foo");
    });
    it("can catch dotnet exceptions", () => {
        assert.throws(() => Test.throwCS("bar"), /Error: bar/);
    });
});
